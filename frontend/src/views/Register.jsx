...
class Register extends React.Component {
  ...
  async handleSubmit() {
    if (this.validate()) {
      ...
      API.post("auth/register", {
        name: this.state.input.authName,
  ...
  validate() {
    ...
    if (!input["authName"]) {
      isValid = false;
      errors["authName"] = "Please enter your name";
    }
    ...
    if (typeof input["authPassword"] !== "undefined") {
      if (!Helper.validatePassword(input["authPassword"])) {
        isValid = false;
        errors["authPassword"] = "Must be minimum 12 characters, at least 1 upper-case alphabet & contains non-alphanumeric character";
      }
    }
      ...
      return (
        <div className="w-100 h-100 overflow-auto text-light bg-primary">
          <div className="position-fixed text-right w-100 p-4 bg-primary auth-nav">
            Already a user?
            <Link to="/auth/login" className="text-light ml-1">
            ...
            <div className="form-group">
              ...
              <div
                className={`input-group ${
                  this.state.errors.authName ? "border-danger" : ""
                }`}
              >
